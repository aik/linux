...
#define TCE32_TABLE_SIZE	((0x10000000 / 0x1000) * 8)
	...
	iommu_init_table(tbl, phb->hose->node, &pnv_ioda1_iommu_ops);
		...
		unsigned shift, unsigned levels, unsigned long *left)
{
	struct page *tce_mem = NULL;
	__be64 *addr, *tmp;
	unsigned long order = max(shift, (unsigned)PAGE_SHIFT) - PAGE_SHIFT;
...
#define ROUND_UP(x, n) (((x) + (n) - 1u) & ~((n) - 1u))
	...
	pr_info("Creating 32-bit TCE table at 0..%08llx, %d levels, TCE table size = %lx\n",
...
static long pnv_pci_ioda2_ddw_query(struct powerpc_iommu *iommu,
		__u32 *tce32_start, __u32 *tce32_size,
		__u32 *windows_supported, __u32 *levels,__u32 *flags)
{
	struct pnv_ioda_pe *pe = container_of(iommu, struct pnv_ioda_pe,
						iommu);
	struct pnv_phb *phb = pe->phb;
	...
	*tce32_start = 0;
	*tce32_size = phb->ioda.m32_pci_base;
	*windows_supported = POWERPC_IOMMU_MAX_TABLES;
	*levels = 5;
	*flags = DDW_PGSIZE_4K | DDW_PGSIZE_64K | DDW_PGSIZE_16M;
